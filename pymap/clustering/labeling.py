...
class ClusterLabeler:
    """Generates descriptive labels and metadata for clusters using OpenAI API."""
    ...
    def generate_cluster_metadata(self, df: pl.DataFrame) -> pl.DataFrame:
        """Generate metadata dataframe with cluster_id, cluster_label, and centroid coordinates."""
        config = Config.load()
        client = OpenAI(api_key=config.openai.openai_api_key)

        cluster_metadata = self._create_cluster_metadata(df, client, config.openai.model_name)
        return cluster_metadata

    def _create_cluster_metadata(self, df: pl.DataFrame, client: OpenAI, model_name: str) -> pl.DataFrame:
        """Create metadata dataframe with cluster_id, cluster_label, and centroids."""
        cluster_ids = self._get_unique_cluster_ids(df)
        metadata_rows = []

        for cluster_id in cluster_ids:
            label = self._generate_label_for_cluster(df, cluster_id, client, model_name)
